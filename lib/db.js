  ...
  if (sql) {
    try {
      ...
      const rows = await sql`
        SELECT id, username, team_name, current_level, highest_level_unlocked, created_at, last_active_at 
        FROM players 
  ...
  if (sql) {
    try {
      ...
      const existingRows = await sql`
        SELECT id, username, team_name, password, current_level, highest_level_unlocked, created_at, last_active_at
        FROM players
      ...
      if (existingRows.length > 0) {
        ...
        const updatedRows = await sql`
          ...
          RETURNING id, username, team_name, password, current_level, highest_level_unlocked, created_at, last_active_at;
        `;
        return updatedRows[0];
      }

      const newRows = await sql`
        INSERT INTO players (username, team_name, password, current_level, highest_level_unlocked, last_active_at)
        VALUES (${trimmedUser}, ${trimmedTeam}, ${trimmedPass}, 0, 0, CURRENT_TIMESTAMP)
        RETURNING id, username, team_name, password, current_level, highest_level_unlocked, created_at, last_active_at;
      `;
      return newRows[0];
    } catch (err) {
    ...
    try {
      ...
      const rows = await sql`
        ...
        SET current_level = ${levelNum},
            ...
            last_active_at = CURRENT_TIMESTAMP
        WHERE LOWER(username) = LOWER(${cleanUser}) OR LOWER(team_name) = LOWER(${cleanUser})
        RETURNING id, username, team_name, current_level, highest_level_unlocked, created_at, last_active_at;
      `;
      if (rows.length > 0) return rows[0];
    } catch (err) {
